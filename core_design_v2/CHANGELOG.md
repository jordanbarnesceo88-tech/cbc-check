...
| R12 | `core_v2 refine 12: client feedback round 1` | (1) «Подход» removed from the header nav. (2) Hero headline now «Оптимизируем, …». (3) New hero subtitle (CBC supports companies across the CIS and the Middle East…). (4) New About paragraph (local + international experience; trade, logistics, IT). (5) Principles redesigned as ledger tiles: ghost outlined index numeral (red on hover), icon box, lift + shadow on hover, staggered second column; hover still reveals the description. (6) About heading is now «Наши принципы». (7) Accounting description reworded to services incl. tax support. (8) Fixed section labels sitting on the same line as the heading — `.label` is now block-level, so «Направления работы» is strictly above the H2 (same fix applied to Contact). (9) The «Оптимизация» approach step removed (4 steps). Same content edits synced to `core_design_v3`. Assets bumped to `?v=8` (v2) / `?v=2` (v3 js). |
| R13 | `core_v2 refine 13: client feedback round 2` | (1) v2 is the only maintained version (v3 stays as an archived experiment). (2) Hero eyebrow: «Комплексное сопровождение бизнеса» (dropped «платформа»). (3) Header logo swapped to the new `logo_dark_core.svg` at 36px (x1.3). (4) Principles reworked: the lead paragraph sits under the «Наши принципы» heading; the cubes are a full-width 4-across row of true squares (270x270 desktop, 2-across below 1024, 1-column on phones); ghost numerals padded inside the cube instead of bleeding off the edge. Assets bumped to `?v=9`. |
| R14 | `core_v2 refine 14: static principle descriptions` | Descriptions under «Профессионализм» etc. are now always visible (hover-reveal removed); hover effects on the cubes neutralized. |
| R15 | `core_v2 fix: remove CSS scroll-behavior:smooth (pin overlap root cause)` | Root cause of the services section overlapping «Наши принципы»: CSS smooth scrolling made `ScrollTrigger.refresh()` measure mid-animation on a scrolled page, skewing every trigger start by −scrollY (measured: start 142 vs true 1546 after a reload with restored scroll; never self-healed; also explains the R11 episode). Fix: removed `html{scroll-behavior:smooth}`; anchors smooth-scroll via JS (`scrollIntoView`, respects reduced-motion); `section[id]{scroll-margin-top:76px}` so anchors land below the fixed header. Header logo returned to 28px. Assets → `?v=10` (legal pages too). |
